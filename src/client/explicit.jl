...
mutable struct ExplicitClient <: AbstractExplicitClient
    ...
    function ExplicitClient(
        host::String, port::Int; secure=false, deadline=10, keepalive=60
    )
        base = BaseDisciplineClient(
            host, port; secure=secure, deadline=deadline, keepalive=keepalive
        )
        compute_client = ExplicitService_ComputeFunction_Client(
            host, port; secure=secure, deadline=deadline, keepalive=keepalive
        )
        gradient_client = ExplicitService_ComputeGradient_Client(
            host, port; secure=secure, deadline=deadline, keepalive=keepalive
        )
...
function set_stream_options!(client::ExplicitClient; kwargs...)
...
function compute_partials(
    client::ExplicitClient, inputs::Dict{String, <:AbstractVector{Float64}}
)
    # Create channels
    request_channel = Channel{PhiloteProto.philote.var"#Array"}(16)
    response_channel = Channel{PhiloteProto.philote.var"#Array"}(16)
